...
namespace NGround {
	...
	ui32 TJsonDocument::GetArraySize() const {
		ENSURE(CurrentValue().IsArray(), "Value is not array");
		return CurrentValue().Size();
	}

	TJsonDocument TJsonDocument::operator[] (ui32 id) {
		return TJsonDocument(*this, CurrentValue()[id]);
	}

	const TJsonDocument TJsonDocument::operator[](ui32 id) const {
		return TJsonDocument(*this, CurrentValue()[id]);
	}
